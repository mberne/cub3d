...
int	player_move(t_struct *as)
{
	...
	if (as->key.front)
	{
		as->player.x = as->player.x + sinf(as->player.rad) * 0.1;
		as->player.y = as->player.y - cosf(as->player.rad) * 0.1;
	}
	if (as->key.back)
	{
		as->player.x = as->player.x - sinf(as->player.rad) * 0.1;
	...
	if (as->key.left)
	{
		as->player.x = as->player.x - cosf(as->player.rad) * 0.1;
		as->player.y = as->player.y - sinf(as->player.rad) * 0.1;
	}
	if (as->key.right)
	{
		as->player.x = as->player.x + cosf(as->player.rad) * 0.1;
